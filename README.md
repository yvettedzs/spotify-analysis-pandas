# spotify-analysis-pandas
An analysis of Kaggle's Top 50 Spotify Tracks 2020 dataset


## The Dataset  
Kaggle's Top 50 Spotify Tracks 2020 is used for the analysis.  
https://www.kaggle.com/datasets/atillacolak/top-50-spotify-tracks-2020   
Spotify's audio features are explained here https://developer.spotify.com/documentation/web-api/reference/get-audio-features  

## The Context  
I am a data analyst working for Spotify. Our team is responsible for content analysis, and the goal of the analysis at hand is to quantify what makes a hit song. 

## The Solution  
Both the .csv file and the Jupyter notebook have been uploaded.  
